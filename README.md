...
Switching never removes a package or cask just because it isn't listed in the `brews` and `casks` arrays, so anything you installed by hand with `brew install` stays put.
The tradeoff is that your machine can drift from the config: a package you installed ad-hoc keeps working, but it won't reappear on a fresh install unless you add it to `configuration.nix`.
If you'd rather have strict reproducibility, set this to `"zap"`, but read `brews` and `casks` first, because the next switch will then uninstall everything not listed there.
